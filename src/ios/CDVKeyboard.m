...
- (void)shrinkViewKeyboardWillChangeFrame:(NSNotification*)notif
{
    ...
    if (CGRectContainsRect(screen, keyboardIntersection) && !CGRectIsEmpty(keyboardIntersection) && _shrinkView && self.keyboardIsVisible) {
        ...
        if (@available(iOS 12, *)) {
            ...
            screen.size.height = screen.size.height -keyboardIntersection.size.height + 22;
    ...
    // I'm sure there's a better way...
    if (@available(iOS 12, *)) {
        CGSize revisedSize = CGSizeMake(self.webView.frame.size.width, self.webView.frame.size.height - keyboard.size.height + 22);
